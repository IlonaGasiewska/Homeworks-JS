...
function totalSpending2014 (){
   return financialData.filter(data => data.detailsOfPayent.date.includes("2014")).reduce((counter, transaction)=>{return counter += parseFloat(transaction.cost)},0)
};

console.log(totalSpending2014()); //46751
...
function EarningsPerCompany(data) {
    ...
    return companies.map(company => {
        ...
        const totalEarnings = transactionsForCompany.reduce((sum, transaction) => sum + parseFloat(transaction.cost), 0);
...
function spendingPerTransactionType(data){
    ...
    return transactionsType.map((transactionType) => { 
       ...
       let totalSpendingByTransactionType = transactionPerType.reduce((sum, transaction) => sum += parseFloat(transaction.cost),0)
